...
document.addEventListener('click', ({target}) => {
  ...
  if (cmd) {
    if (cmd === 'apply') {
      ...
      chrome.storage.local.get({
        'test': 'http://localhost:8080/device.php'
      ...
      chrome.storage.local.get({
        'test': 'https://mufid.azazil.site/mufid30'
      }, prefs => chrome.tabs.create({
        url: prefs.test
      }));
    }
      ...
      chrome.storage.local.get({
        'test': 'chrome://settings/clearBrowserData'
      }, prefs => chrome.tabs.create({
        url: prefs.test
      }));
    }
	    else if (cmd === 'test2') {
      ua.select(); document.execCommand("delete");
    }
		    else if (cmd === 'test3') {
      ua.select(); document.execCommand("insertText");
    }
    if (cmd) {
